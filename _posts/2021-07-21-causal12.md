...
具体来说，已知源人群数据 $P(y,t,...)$ 和 目标人群数据 $P^*(y,t,...)$ ，给出源模型 $P(y\vert do(t),x)$ ，我们的目标是得到对应模型 $P^*(y\vert do(t),x)$ ，判断两者是否相等

我们首先引入 Selection Diagrams 的概念，它允许两个分布有不同的causal mechanism。具体方法是增加节点 S 指向原图节点。此时我们有 $P^*(y \vert do(t), x) \triangleq P(y \vert do(t), x, s^*)$

这里有一些不同的 Transportability，列举如下
* Direct Transportability  
  我们有 $P(y \vert do(t), x) = P^*(y \vert do(t), x) \quad if\ Y \perp \!\!\! \perp_{G_{\overline{T}}} S\vert T,X$  
  这是利用了 do-calculus 的 rule1 进行化简，表明两组人群的causal effect 完全相同
* Trivial Transportability  
  如果我们无法得到Direct Transportability，但可以接触到目标人群的观测数据，那么我们可以用观测数据来 identify estimand，即 $P^*(y \vert do(t), x) = P^*(y \vert t, x)$ 这利用的是 do-calculus 的 rule2
