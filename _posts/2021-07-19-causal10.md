...
首先考虑最简单的双变量情况，即 $X\rightarrow Y$ 和 $Y\rightarrow X$ 。如果从马尔科夫等价的角度来看，我们得到的essential graph是相同的，因此无法区分这两种情况。如果从结构方程的角度来看，前一种情况对应 $Y=f_Y(X,U_Y),\ X \perp \!\!\! \perp U_Y$ ，后一种情况对应 $X=f_X(Y,U_X),\ Y \perp \!\!\! \perp U_X$ ，此时同样无法进行区分  

所以在这里我们需要对 parametric form 做一些假设。
* Linear Non-Gaussian Assumption：第一种假设噪声为线性非高斯分布的，即 $Y := f(X) + U,\ X \perp \!\!\! \perp U$ ，$U$为非高斯分布。此时根据现有研究，我们有如下定理  
  linear non-Gaussian 假设下，如果真实的 SCM 为 $Y=f(X)+U,\ X \perp \!\!\! \perp U$ ，那么此时不存在能产生一致 P(x,y) 数据的相反方向的 SCM，即 $X=g(Y)+\tilde{U},\ Y \perp \!\!\! \perp \tilde{U}$ ，具体证明可见课程教材
* Nonlinear additive noise assumption：第二种假设噪声为加性的，方程时非线性的，即 $Y := f(X) + U,\ X \perp \!\!\! \perp U$ ，$f$ 是非线性方程。根据现有研究，此时结合Markov assumption, causal sufficiency, acyclicity assumption, 我们可以 identify the causal graph  
  此外，我们也可以在外面继续嵌套非线性方程，将线性噪声转换为非线性噪声，即 $Y := g(f(X) + U),\ X \perp \!\!\! \perp U$ ，则称为 Post-Nonlinear Assumption
