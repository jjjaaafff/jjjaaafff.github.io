...
2. 第二种解释方式是梯度更新的角度。我们的原目标为极大似然 $max_{\theta} \ln P(X\vert\theta)$ ，我们对优化目标求导可以得到
...
1. 第一种证明用到了 Jensen 不等式，即对于凸函数 $f(\cdot)$ 而言，我们有 $E[f(x)] \ge f(E[x])$ ，而
...
1. 初始化，确定模型初始值 $\pi_k, \mu_k, \Sigma_k$
2. E-step: 根据现有模型值，计算 $\gamma$

$$\gamma(z_{nk}) = \frac{\pi_kN(x_n\vert \mu_k,\Sigma_k)}{\sum_{i=1}^K\pi_iN(x_n\vert \mu_i,\Sigma_i)}$$

3. M-step: 根据计算的 $\gamma$ 值重新估计更新模型参数
...
4. 收敛之后，我们就可以去估计 log-likelihood
...
当 $\epsilon$ 逐渐减小直至趋近于零时，$||x_n-\mu_i||^2$ 最小的那一项最晚降为零，因此只有这一项为一，其余都很快地变为零。即此时 $\gamma(z_{nk}) \rightarrow \delta_{nk}$ ，每个数据点都被唯一地划分到均值最小的对应聚类中。此时 M-step 中均值的更新退化为
